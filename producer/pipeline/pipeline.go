...
func Run(ctx context.Context) error {
	...
	userStore, err := user.NewStore(cfg.UserStoragePath)
	if err != nil {
		return fmt.Errorf("create user store: %w", err)
	}

	activeUser, err := userStore.Active()
	if err != nil {
		return fmt.Errorf("no active user: run 'producer login <email>' first: %w", err)
	}
	...
	watcher := user.NewWatcher(userStore, 5*time.Second, func(u user.User) {
		identityMu.Lock()
		...
		logger.Info("active user changed",
			zap.String("user_id", u.ID),
			zap.String("username", u.DisplayName),
		)
	})
	...
	logger.Info("starting producer",
		...
		zap.String("active_user_id", activeUser.ID),
		zap.String("active_username", activeUser.DisplayName),
	)

	normalizerService := normalizer.New(activeUser.ID)
...
func runCaptureLoop(
	...
	capturePhysicsFn := func(runCtx context.Context) (int, error) {
		...
		id := *identity
		identityMu.Unlock()
		return capturePhysics(runCtx, reader, normalizerService, serializer, batcher, cfg.TopicPhysics, id)
	}
	...
	captureGraphicsFn := func(runCtx context.Context) (int, error) {
		...
		id := *identity
		identityMu.Unlock()
		return captureGraphics(runCtx, reader, normalizerService, serializer, batcher, cfg.TopicGraphics, id)
	}
	...
	captureStaticFn := func(runCtx context.Context) (int, error) {
		...
		id := *identity
		identityMu.Unlock()
		return captureStatic(runCtx, reader, normalizerService, serializer, batcher, cfg.TopicStatic, id)
	}
